package com.liyazhou.dragview.customview;
...
import com.liyazhou.dragview.R;

/**
 * Created by liyazhou on 16/12/4.
 */
...
public class SwitchView extends View {
    ...
    public SwitchView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.SwitchView, defStyleAttr, R.style.def_switch_view);
        int indexCount = typedArray.getIndexCount();
        for(int i = 0; i < indexCount; i++) {
            int att = typedArray.getIndex(i);
            switch (att) {
                case R.styleable.SwitchView_switch_bg_close_color:
                    switchViewBgCloseColor = typedArray.getColor(att, Color.BLACK);
                    break;
                case R.styleable.SwitchView_switch_bg_open_color:
                    switchViewBgOpenColor = typedArray.getColor(att, Color.BLACK);
                    break;
                case R.styleable.SwitchView_switch_bg_strock_width:
                    switchViewStrockWidth = typedArray.getDimensionPixelOffset(att, 0);
                    break;
                case R.styleable.SwitchView_switch_strock_color:
                    switchViewStrockColor = typedArray.getColor(att, Color.BLACK);
                    break;
                case R.styleable.SwitchView_switch_ball_color:
                    switchViewBallColor = typedArray.getColor(att, Color.BLACK);
                    break;
            }
            typedArray.recycle();
            initData();
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        Log.e("haha", "onSizeChanged:" + w + ";" + h + ";" + oldw + ";" + oldh);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Log.e("haha", "onMeasure:" + MeasureSpec.getSize(widthMeasureSpec) + ";" + MeasureSpec.getSize(heightMeasureSpec));
    ...
    protected void onDraw(Canvas canvas) {
        Log.e("haha", "onDraw:" + canvas.getHeight() + ";" + canvas.getWidth());
        super.onDraw(canvas);
        drawSwichBg(canvas);
